...
filter ::
  ...
  -> List a
filter p =
  foldRight (\x acc -> if p x then x :. acc else acc) Nil
...
seqOptional ::
  ...
  -> Optional (List a)
seqOptional =
  foldRight optionalConcat (Full Nil)
  where
    optionalConcat Empty _ = Empty
    optionalConcat _ Empty = Empty
    optionalConcat (Full a) (Full xs) = Full (a :. xs)
